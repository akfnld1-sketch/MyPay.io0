...
function getPayData(){
  ...
  // 월 총 근무시간 (기본 + 연장)
  const monthlyHours = normalH + totOT + satH + sunH + holidayH;
  // 자동계산 후 override 적용 (60시간 기준 4대보험 판단)
  const _ins = calc4Insurance(grossPay, monthlyHours);
  ...
  return {basePay, normalH, monthlyHours, twd, wDays, lDays, halfDays, absDays,
 ...
 */
function calc4Insurance(grossPay, monthlyHours){
  const gp = Math.round(grossPay);

  // ── 월 60시간 기준 판단 ──
  // 60시간 이하: 단기근로자 → 고용보험만 적용
  // 60시간 초과: 4대보험 전체 적용 (국민연금+건강보험+장기요양+고용보험)
  const isShortTerm = (monthlyHours !== undefined) && (monthlyHours <= 60);
  ...
  if(isShortTerm){
    // 단기근로 (월 60h 이하): 고용보험만
    return { np:0, hi:0, ltc:0, ei, total:ei, isShortTerm:true };
  }

  // 일반근로 (월 60h 초과): 4대보험 전체
  // 국민연금: 기준소득월액 상·하한 적용
  ...
  return { np, hi, ltc, ei, total, isShortTerm:false };
}
...
function renderIncomePage(){
  ...
  // ── N잡 세금 구분 계산 ──
  // 알바(shortAlba)/편의점(convenience) → 근로소득 → 고용보험만 (3.3% 아님)
  // 프리랜서(freelancer)/배달(delivery)/대리(driver)/기타 → 사업소득 → 3.3%
  ...
  const albaEI     = Math.round(albaGrossForTax * INS.EI); // 알바 고용보험 0.9%
  const njobBizTax = Math.round(bizGross * 0.033);         // 사업소득 3.3%
  const njobTax    = albaEI + njobBizTax;                  // 전체 공제 합산
  ...
  <div style="background:var(--surface);border:1px solid var(--border);border-radius:12px;
    ...
    <div style="display:none;padding:14px 16px;">
      ...
      <div style="display:flex;flex-direction:column;gap:5px;">
        ...
        </div>`:''}        ${employeeData.ins && employeeData.ins.isShortTerm !== undefined ? `
        <div style="margin:6px 0;padding:7px 10px;border-radius:8px;
          background:${employeeData.ins.isShortTerm ? 'rgba(61,214,140,.1)' : 'rgba(255,107,107,.1)'};
          border:1px solid ${employeeData.ins.isShortTerm ? 'rgba(61,214,140,.3)' : 'rgba(255,107,107,.3)'};">
          <div style="font-size:11px;font-weight:700;color:${employeeData.ins.isShortTerm ? 'var(--green)' : 'var(--red)'};">
            ${employeeData.ins.isShortTerm
              ? '✅ 월 60시간 이하 · 고용보험만 적용'
              : `⚠️ 월 ${Math.round(employeeData.monthlyHours||0)}시간 초과 · 4대보험 전체 적용`}
          </div>
          ${!employeeData.ins.isShortTerm ? `
          <div style="font-size:10px;color:var(--text2);margin-top:3px;">
            국민연금 ${(employeeData.ins.np||0).toLocaleString()}원 · 건강보험 ${(employeeData.ins.hi||0).toLocaleString()}원 · 장기요양 ${(employeeData.ins.ltc||0).toLocaleString()}원 · 고용보험 ${(employeeData.ins.ei||0).toLocaleString()}원
          </div>
          <div style="font-size:10px;color:#ff6b35;margin-top:4px;padding:5px 8px;border-radius:6px;background:rgba(255,107,53,.08);border:1px solid rgba(255,107,53,.2);line-height:1.6;">
            🚨 월 60시간 초과 시 이전 근무분 소급 정산이 발생할 수 있어요!<br>
            첫 초과 명세서에서 이전 미공제분이 한꺼번에 빠질 수 있으니 미리 준비하세요.
          </div>` : ''}
          <div style="font-size:10px;color:var(--text3);margin-top:5px;padding-top:5px;border-top:1px dashed var(--border);line-height:1.5;">
            ℹ️ 4대보험 적용 기준은 회사 정책에 따라 다를 수 있어요.<br>실제 명세서와 다를 경우 직접 수정하거나 노무사 상담을 받으세요.
          </div>
        </div>` : ''}
  ...
  function makeNjobSection(label, icon, color, items, gross){
    ...
    // 알바는 고용보험(0.9%) 공제, 프리랜서/배달은 3.3% 공제
    const isBizIncome  = label.includes('프리랜서') || label.includes('배달') || label.includes('대리');
    const isAlbaIncome = label.includes('알바');
    const netLabel = isBizIncome ? '3.3% 공제 후' : isAlbaIncome ? '고용보험(0.9%) 공제 후' : '소계';
    const netAmt   = isBizIncome ? Math.round(gross * 0.967) : isAlbaIncome ? Math.round(gross * 0.991) : gross;
    const inner = items.length>0 ? rows + `
      <div style="display:flex;justify-content:space-between;padding:8px 0;border-top:1px solid var(--border);margin-top:4px;">
        <span style="font-size:12px;color:var(--text2);">${netLabel}</span>
        <span style="font-size:14px;font-weight:700;color:var(--green);">${netAmt.toLocaleString()}원</span>
      </div>` :
  ...
  const taxHTML = njobGross>0 ? `
    <div style="background:var(--surface);border:1px solid var(--border);border-radius:12px;padding:14px 16px;margin-bottom:16px;">
      ...
      <div style="font-size:12px;color:var(--text2);line-height:2;">
        ...
        <div style="display:flex;justify-content:space-between;padding:4px 0;border-bottom:1px solid var(--border);">
          <span style="padding-left:8px;">└ 고용보험 공제 (0.9%)</span><span style="font-weight:700;color:var(--red);">-${albaEI.toLocaleString()}원</span>
        </div>`:''}
        ${bizGross>0?`
        <div style="display:flex;justify-content:space-between;padding:4px 0;border-bottom:1px solid var(--border);">
          <span>💻 프리랜서·배달 (사업소득)</span><span style="font-weight:700;color:var(--accent);">${bizGross.toLocaleString()}원</span>
        </div>
        <div style="display:flex;justify-content:space-between;padding:4px 0;border-bottom:1px solid var(--border);">
      ...
      <div style="margin-top:8px;padding:8px 10px;background:rgba(255,209,102,.07);border-radius:7px;font-size:11px;color:var(--text3);line-height:1.8;">
        ⚠️ 프리랜서·배달 수입은 <b>5월 종합소득세 신고</b> 필수<br>
        알바 근로소득은 회사에서 연말정산 처리해요<br>
        ℹ️ 세금 방식은 계약 형태에 따라 다를 수 있어요
      </div>
    </div>` : '';
